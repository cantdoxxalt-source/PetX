...
function CaChip({ ca }: { ca: string }) {
  ...
  const truncated = ca.slice(0, 4) + "…" + ca.slice(-4);
  ...
  return (
    <button
      onClick={copy}
      title={ca}
      className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-md border border-border bg-surface-warm hover:bg-surface-soft transition-colors monoText text-xs text-muted hover:text-foreground"
    >
      <span>CA: {truncated}</span>
      {copied ? (
